...
def generate_synthetic_entry(exercise):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            metrics["retry_details"].append(retry_info)
            print(f"    [{exercise}] Invalid JSON (attempt {attempt+1}/{MAX_RETRIES})")
...
def save_summary():
    ...
    print(f"⏱  Durée totale : {GLOBAL_STATS.get('total_duration_human', 'N/A')}")
    print()
...
def main():
    ...
    print(f"  Max retries par sample : {MAX_RETRIES}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n  Arrêt d'urgence. Sauvegarde des métriques...")
        save_summary()
